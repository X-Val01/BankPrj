...
public class ATMClient {
    ...
    private enum ATMCycle { ENTER_CUST_ID, PERFORM_OPERATION }
    private ATMCycle cycle = ATMCycle.ENTER_CUST_ID;
    private enum ATMOperation { UNKNOWN, GET_BALANCE, DEPOSIT, WITHDRAW }
    ...
    private static final String ENTER_CUSTOMER_TEXT
    ...
    private void performOperation() {
        ...
        switch ( operation ) {
            case GET_BALANCE: {
                outputArea.append("Your account balance is: "
                        + account.getBalance() + "\n");
                break;
            }
            case DEPOSIT: {
                try {
                    ...
                    outputArea.append("Deposit amount is not a number: "
                            + entryField.getText());
                }
                break;
            }
            case WITHDRAW: {
                try {
                    ...
                    outputArea.append("Your withdrawal of "
            ...
            switch ( cycle ) {
                case ENTER_CUST_ID: {
                    ...
                    try {
                        ...
                        if ( customer == null ) {
                            outputArea.append("Customer ID is not valid for this Bank: "
                                    + entryField.getText());
                        } else {
                            account = customer.getAccount(0);
                            outputArea.append("Welcome " + customer.getFirstName()
                            + " " + customer.getLastName() + "\n\n");
                            cycle = ATMCycle.PERFORM_OPERATION;
                            enableActionButtons(true);
                        }
                    } catch (NumberFormatException nfe) {
                        outputArea.append("Customer ID is not a number: "
                                + entryField.getText());
                    }
                    entryField.setText("");
                    break;
